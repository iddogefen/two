...
vif = rep(0, n.loop)
corr.mat= rep(0, n.loop)
desmats = array(0,c(run.length, 3,  n.loop))
trialtypes = array(0,c(2*ntrials.each,  n.loop))
...
for (i in 1:n.loop){
  ...
  corr = cor.test(faces,houses)
  corr.mat[i] = as.numeric(corr$estimate)
  desmats[,,i] = des.mat
  vif[i] = diag(solve(cor(des.mat[,2:3])))[1]
}
