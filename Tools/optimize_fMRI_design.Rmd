...
# we first decide on an iti vector - since we want to make sure the same vector will appear in all runs and all versions of the task. 
curr_iti = rtruncexp(nrow(curr_df), 
                     rate=iti_params$rate, 
                     a=iti_params$min, 
                     b=iti_params$max)
curr_iti = round_any(as.numeric(curr_iti), 0.25)*1000 # convert iti to jumps of 500ms (so length will be divided by TR)
mean(curr_iti) # 2625
max(curr_iti) # 5000
min(curr_iti) # 1500
hist(curr_iti)
...
# run length
(sum(chosen_iti_vector) + nrow(curr_df)*stimulus_duration)/60000
...
while (keep.looking == 1){
  ...
  keep.looking = max(length.repeats) > max_stim_repeats
}  #end while
...
n_reps <- 100
n_sims <- n_reps*n_versions*n_runs
iti_mat <- matrix(data=NA, nrow=n_sims, ncol=length(chosen_iti_vector)+2)
corr_mat <- matrix(data=NA, nrow=n_sims, ncol=1+2)
efficiency_mat <- matrix(data=NA, nrow=n_sims, ncol=1+2)
VIF_mat <- matrix(data=NA, nrow=n_sims, ncol=1+2)
cns_mat <- matrix(data=NA, nrow=n_sims, ncol=1+2)
counter = 1;

for (v in 1:n_versions){
  for (r in 1:n_runs){
    # take current run
    curr_df <- df[df$run==r, ] %>% relocate(c(!!sym(event_column),iti), .after="PID") 
    
    # we sample order of trials and order of itis, until we make sure that the critical stimulus does not repeat more than two times, and also the longer itis do not repeat more than 3/4 trials
      keep.looking = 1
      while (keep.looking == 1){
        curr_df <- curr_df %>%
          group_by(block) %>%
          slice(sample(1:n())) 
        stimulus.length.repeats = rle(as.vector(unlist(curr_df[event_column])))$lengths # The rle function breaks a sequence into lengths and values
        # Keep going if the max number of repeats is larger than 2
        keep.looking= max(stimulus.length.repeats) > max_stim_repeats
      }  #end while
    
      for (i in 1:n_reps){
      curr_iti = sample(chosen_iti_vector)
      curr_df$iti <- curr_iti
      curr_design <- compute_design_matrix(TR=1.5, 
                                         response.duration=stimulus_duration,
                                         event.column=event_column, 
                                         current.df = curr_df)
      iti_mat[counter,] <- c(curr_iti,v,r)
      corr_mat[counter,] <- c(curr_design$correlation,v,r)
      efficiency_mat[counter,] <- c(curr_design$efficiency,v,r)
      VIF_mat[counter,] <- c(as.numeric(curr_design$VIF[1]),v,r)
      counter = counter + 1
...
curr_df <- swap_itis(curr_df, "iti", 5, 10)
curr_design <- compute_design_matrix(TR=1.5, 
                                     response.duration=stimulus_duration,
                                     event.column=event_column, 
                                     current.df = curr_df)
